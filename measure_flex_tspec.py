...
for img in imgs:
	...
	xs.append([cms[0], sds[0], cor[0], sweep]) # centroid mean, centroid stddev, correced mean (after flexure compensation)
...
xs = np.array(xs)
ys = np.array(ys)
pas = np.array(pas) # position angles
els = np.array(els) # elevations

xd = xs[:,0] # zeroth column of xs : centroid mean
xe = xs[:,1] # first column of xs : centroid stddev
xc = xs[:,2] # second column of xs : corrected mean
yd = ys[:,0]
ye = ys[:,1]
yc = ys[:,2]
sw = xs[:,3]
